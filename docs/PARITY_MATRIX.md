...
| Comissões (público) | `comissoes`, `comissoes/[leagueId]/**`, `CollectiveCatalogPage.tsx`, `CollectivePublicDetailClient.tsx` | Catálogo ordenado por vendas + página com as quatro abas | Nenhum | Sim: mesma base das ligas + `users` por turma e `produtos.vendidos`/`likes` | Concluído no M6. Configuração (`/comissoes/configurar`) é o M7. |
| Diretório (público) | `diretorio`, `diretorio/[leagueId]/**`, `PrimaryDirectoryPage.tsx` | Raiz com registro primário + página parametrizada, ambas com as quatro abas | Nenhum | Sim: mesma base das ligas | Concluído no M6. Configuração (`/diretorio/configurar`) é o M7. |
| Gestão de coletivos | `ligas/**`, `comissoes/configurar/**`, `diretorio/configurar/**` | Ausente | N/A | Não | M7: gestão, financeiro, frequência, produtos, loja e membros das três áreas. |
