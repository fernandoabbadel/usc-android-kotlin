...
export default function ConfigPedidoEventoIngressoDetalhePage() {
  const params = useParams<{
    status?: string | string[];
    pedidoId?: string | string[];
    ticketToken?: string | string[];
  }>();
  const status = firstParamValue(params.status) || "aprovados";
  const pedidoId = firstParamValue(params.pedidoId);
  const ticketToken = firstParamValue(params.ticketToken);
  ...
  return (
    <div className="min-h-screen bg-[#050505] pb-24 font-sans text-white">
      <header className="sticky top-0 z-30 flex items-center gap-4 border-b border-white/5 bg-[#050505]/90 p-4 backdrop-blur-md">
        <Link
          href={tenantPath(`/configuracoes/pedidos/eventos/${status}/${pedidoId}`)}
          className="-ml-2 rounded-full p-2 text-zinc-400 transition hover:bg-zinc-900 hover:text-white"
        >
          <ArrowLeft size={24} />
        </Link>
        <h1 className="text-xl font-black italic uppercase tracking-tighter text-white">Detalhe do ingresso</h1>
      </header>
      <PedidosByTypePage
        tab="eventos"
        detailPedidoId={pedidoId}
        detailTicketToken={ticketToken}
